...
## foreign key( como se relacionan dos tablas)
    def __init__(self, Nombre, Apellido, id_number, contrasenia):
        ...
        sentencia = "SELECT * FROM clientes WHERE id_number = " + "'" + id_number + "'"
        cursor.execute(sentencia)
        cliente = cursor.fetchone()
        if cliente is None:
            conexion = sqlite3.connect('C:/Users/franc/PycharmProjects/Practicas fundamentos/App_libros/Applibros.db')
            cursor = conexion.cursor()
            m = (Nombre, Apellido, id_number, contrasenia)
            sentencia_sql = "INSERT INTO clientes VALUES (?, ?, ?, ?)"
            cursor.executemany(sentencia_sql, [m])
            conexion.commit()
            conexion.close()

    # def eliminar_clientes(self, id_number, contrasenia):
    #     self.id_number = str(id_number)
    #     self.contrasenia = contrasenia
    #     conexion = sqlite3.connect('C:/Users/franc/PycharmProjects/Practicas fundamentos/App_libros/Applibros.db')
    #     cursor = conexion.cursor()
...
class libros:
    ...
    def __init__(self, titulo_obra, genero, paginas, precio, id_number):
        ...
        conexion = sqlite3.connect('C:/Users/franc/PycharmProjects/Practicas fundamentos/App_libros/Applibros.db')
        cursor = conexion.cursor()
        sentencia = "SELECT * FROM Libros WHERE id_number = " + "'" + id_number + "' and titulo_obra = " + "'" + titulo_obra + "'"
        cursor.execute(sentencia)
        Libro = cursor.fetchone()
        if Libro is None:
            conexion = sqlite3.connect('C:/Users/franc/PycharmProjects/Practicas fundamentos/App_libros/Applibros.db')
            cursor = conexion.cursor()
            x = (titulo_obra, genero, paginas, precio, id_number)
            sentencia_sql = "INSERT INTO clientes VALUES (?, ?, ?, ?, ?)"
    ...
    def eliminar_libros(self, id_number, titulo_obra):
        self.id_number = str(id_number)
        self.titulo_obra = titulo_obra
        conexion = sqlite3.connect('C:/Users/franc/PycharmProjects/Practicas fundamentos/App_libros/Applibros.db')
        cursor = conexion.cursor()
...
class libros_usados(libros):
    ...
    def __init__(self, titulo_obra, genero, paginas, precio, id_number, condicion, estado, tiempo_de_uso):
        ...
        conexion = sqlite3.connect('C:/Users/franc/PycharmProjects/Practicas fundamentos/App_libros/Applibros.db')
        cursor = conexion.cursor()
        sentencia = "SELECT * FROM Libros_usados WHERE id_number = " + "'" + id_number + "' and titulo_obra = " + "'" + titulo_obra + "'"
        cursor.execute(sentencia)
        Libro = cursor.fetchone()
        if Libro is None:
            conexion = sqlite3.connect('C:/Users/franc/PycharmProjects/Practicas fundamentos/App_libros/Applibros.db')
            cursor = conexion.cursor()
            n = (titulo_obra, genero, paginas, precio, id_number, condicion, estado, tiempo_de_uso)
            sentencia_sql = "INSERT INTO clientes VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
